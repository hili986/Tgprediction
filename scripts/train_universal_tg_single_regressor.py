...
def choose_model(name: str, random_state: int = 42):
    ...
    if key == "physics_homo_correction_strong":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
            ...
            high_dim_kernel_weight=0.0,
            homo_correction=True,
            homo_correction_lambda=0.1,
            homo_correction_landmarks=1800,
        )
    if key == "physics_homo_local_light":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
    ...
    if key == "physics_homo_local_fox_cal":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
    ...
    if key == "physics_homo_local_fox_nonhomo_cal":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
    ...
    if key == "physics_homo_local_fox_wmean_nonhomo_cal":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
            ...
            homo_correction_lambda=0.1,
            homo_correction_landmarks=1800,
            final_calibration_columns=("endpoint_tg_fox_c", "endpoint_tg_weighted_mean_c"),
            final_calibration_lambda=0.1,
            final_calibration_gate_column="is_homopolymer",
            final_calibration_gate_invert=True,
        )
    if key == "physics_homo_local_fox_nonhomo_cal_lowfox_shrink":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
    ...
    if key == "physics_homo_local_fox_pred_delta_nonhomo_cal_lowfox_shrink":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
    ...
    if key == "physics_additive_shrink":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
            ...
            additive_kernel_group_weights=(0.20, 0.08),
        )
    if key == "physics_additive_sum":
        return PhysicsResidualKernelRegressor(
            n_landmarks=1200,
            prior_lambda=0.5,
            residual_lambda=2.0,
            ...
            additive_kernel_group_weights=(0.20, 0.05),
            combine_additive_kernels=True,
            base_kernel_weight=1.0,
        )
    if key == "histgradient":
...
def load_polyinfo_records(path: Path, factory: ComponentFeatureFactory, limit: Optional[int] = None) -> tuple[list[PolymerRecord], int]:
    ...
    for idx, row in frame.iterrows():
        ...
        if raw_sample_id is None or pd.isna(raw_sample_id) or not str(raw_sample_id).strip():
            coid = str(row.get("COID", "")).strip()
            sample_token = f"{coid}:{idx}" if coid else str(idx)
        else:
            sample_token = str(raw_sample_id).strip()
        ...
        try:
            records.append(
                PolymerRecord(
                    sample_id=f"polyinfo:{sample_token}",
...
def make_estimator(model_name: str, random_state: int):
    ...
    if isinstance(model, PhysicsResidualKernelRegressor):
        return model
    return Pipeline(
        [
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", default="catboost", choices=["catboost", "extratrees", "histgradient", "xgboost", "lightgbm", "physics_kernel", "physics_multikernel", "physics_local", "physics_local_light", "physics_hybrid_balanced", "physics_homo_correction", "physics_homo_correction_strong", "physics_homo_local_light", "physics_homo_local_fox_cal", "physics_homo_local_fox_nonhomo_cal", "physics_homo_local_fox_wmean_nonhomo_cal", "physics_homo_local_fox_nonhomo_cal_lowfox_shrink", "physics_homo_local_fox_pred_delta_nonhomo_cal_lowfox_shrink", "physics_additive_kernel", "physics_additive_shrink", "physics_additive_sum"])
